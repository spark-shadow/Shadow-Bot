...
MyPnky.addCommand({pattern: 'install ?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN, dontAddCommandList: true}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        if (plugin_name.length >= 1) {
            plugin_name = "spark_" + plugin_name[1];
        } else {
            plugin_name = "spark_" + Math.random().toString(36).substring(8);
        }
        ...
        if (find.includes(plugin_name)) {
            ...
            if (!match[1].includes('phaticusthiccy') && DEG.level > 99) {
             ...
             if (!match[1].includes('SPARK-SHADOW') && DEG.level > 99) {
                await message.client.sendMessage(message.jid,LANG.limit + DEG.level + '_', MessageType.text)
                fs.unlinkSync('/root/WhatsAsenaDuplicated/plugins/' + plugin_name + '.js')
            }
            else if (!match[1].includes('phaticusthiccy') || (!match[1].includes('SPARK-SHADOW') && DEG.level < 100)) {
...
MyPnky.addCommand({pattern: 'plugin$', fromMe: true, dontAddCommandList: true, desc: Lang.PLUGIN_DESC}, (async (message, match) => {
    ...
    if (plugins.length < 1) {
        ...
        plugins.map(
            (plugin) => {
                let vf = plugin.dataValues.url.includes('SPARK-SHADOW') ? msg : unmsg
